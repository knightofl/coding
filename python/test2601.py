...
rnn_train = tf.keras.preprocessing.sequence.TimeseriesGenerator(
    data=x_train, targets=x_train, length=10, batch_size=1)
print(rnn_train[0], len(rnn_train))

features = 1
model = tf.keras.Sequential()
model.add(tf.keras.layers.SimpleRNN(128, input_shape=(10, features)))
...
rnn_test = tf.keras.preprocessing.sequence.TimeseriesGenerator(
    data=x_test, targets=x_test, length=10, batch_size=1)
